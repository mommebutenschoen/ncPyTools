  ...
  def _writeNCDF(self,compress=True):
    ...
    for k,v in self.variables.items():
        ...
        if "fill_value" in v.keys():
          nc.createVariable(k,v["type"],dims,zlib=compress,complevel=compress,fill_value=float(v["fill_value"]))
        else:
          nc.createVariable(k,v["type"],dims,zlib=compress,complevel=compress)
    ...
    for k,v in self.variables.items():
        ...
        if v["dimensions"]:
          ...
          Slice=[ slice(None,self.shape[d]) for d in v["dimensions"].split(",") ]
          data=self.variables[k]["value"]
          if type(data) in (type(0.),type(0)):
              nc.variables[k][Slice] = ones(Shape)*conv(self.variables[k]["value"])
          else:
              nc.variables[k][Slice] = data
        else: #if dimensions undefined define as scalar
          nc.variables[k][:] = conv(self.variables[k]["value"])
    nc.close()
    print("Done.")
...
if __name__=="__main__":
   parser=ArgumentParser(usage='usage: %prog input.yaml')
   options,args=parser.parse_args()
   nc=netCDFTemplate(args[0])
   nc()
